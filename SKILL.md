...
description: "控制 Windows Chrome 插件，打开网页、抓取亚马逊卖家精灵数据。支持 click/click_text/type/navigate/get_html 等完整命令。默认端口 19000。"
---

## ✅ 核心优势

- **完全兼容 OpenClaw 工具**：支持 `click`, `click_text`, `type`, `navigate`, `get_html`, `screenshot` 等命令
- **默认端口 19000**：`server.py` 默认监听 `0.0.0.0:19000`，可通过 `SERVER_PORT` 环境变量覆盖
...
| `browser(action="screenshot", ...)` | ❌ 端口冲突 | 用 `cmd('screenshot')` |

**✅ 最佳实践：Python WS 命令直接操作**
...
3. **server 崩了**  
   用 `monitor.sh` 保活，或手动重启：`nohup python3 server.py > server.log 2>&1 &`
...
| 文件 | 说明 |
|------|------|
| `server.py` | WebSocket relay server（默认端口 19000，可用 `SERVER_PORT` 覆盖）|
| `background.js` | Chrome extension 逻辑 |
| `start-server.sh` | 启动脚本（nohup + monitor.sh）|
| `test_relay.py` | 连接测试脚本 |
...
- **最新代码**: https://github.com/mkz0930/chrome-control-amz
...
- Date: 2026-03-13
- **Last Updated**: 2026-03-16（默认端口统一为 19000）
